...
function parseId(id?: string) { const n = Number(id); return Number.isFinite(n) && n > 0 ? n : null; }
...
function normalizeUser(u: any) {
  ...
  return {
    ...
    website: u?.website ?? u?.user_url ?? "",
    socials: u?.socials ?? {},
    role: u?.role ?? u?.wp_role ?? null,
  };
}
...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json(normalizeUser(profile), { headers: { "Cache-Control": "no-store" } });
  } catch (e: any) {
...
/* PATCH */
const AvatarUrlSchema = z.string().trim().refine((v) => /^https?:\/\//i.test(v) || v.startsWith("/"), { message: "Invalid URL" });
const HttpUrl = z.string().trim().url().refine(u => /^https?:\/\//i.test(u), "Only http(s) URLs allowed");
function socialSchemaFor(key: SocialKey): z.ZodType<string> {
  if (key === "x_username") {
    return z.union([z.string().trim().regex(/^[A-Za-z0-9_]{1,15}$/, "Invalid username"), z.literal("")]) as unknown as z.ZodType<string>;
  }
  return z.union([HttpUrl, z.literal("")]) as unknown as z.ZodType<string>;
}
...
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { id } = await ctx.params; const userId = parseId(id);
    if (!userId) return NextResponse.json({ error: "Bad id" }, { status: 400 });
    ...
    if (!admin && data.role) return NextResponse.json({ error: "Only admin can change role" }, { status: 403 });

    const socialsPayload = data.socials ? Object.fromEntries(Object.entries(data.socials).map(([k,v]) => [k, (v ?? "") as string])) : undefined;

    await updateUserProfileRepo({
      userId,
      email: data.email, website: data.website,
      first_name: data.first_name, last_name: data.last_name,
      nickname: data.nickname, display_name: data.display_name,
      bio: data.bio,
